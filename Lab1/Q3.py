...
def Q_learning(env, dis_factor, iter):
    ...
    for i in range(iter):
        ...
        record2.append(np.max(Q[env.map[(2,2,2,3)]]))
        record3.append(np.max(Q[env.map[(1,0,2,2)]]))
    
    return Q, record, record2, record3
...
Q, record, record2, record3 = Sarsa(maze, 0.8, iter_)

print(Q)

np.save('record1.npy', record)
np.save('record2.npy', record2)
np.save('record3.npy', record3)
...
plt.plot(np.linspace(0,len(record)-1,len(record)),record2)
plt.legend('state2')
plt.show()


plt.plot(np.linspace(0,len(record)-1,len(record)),record3)
plt.legend('state3')
plt.show()
